...
async function envoyerResend({ to, subject, text, html, from, replyTo }) {
  const body = {
    from,
    to: [to],
    subject,
...
async function envoyerBrevo({ to, subject, text, html, from, replyTo }) {
 ...
 */
export async function envoyerEmail({ to, subject, text, html, from = EXPEDITEUR_DEFAUT, replyTo = [] }) {
  ...
  const donnees = { to, subject, text, html, from, replyTo };
  try {
