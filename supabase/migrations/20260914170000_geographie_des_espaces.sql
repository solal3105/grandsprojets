-- ============================================================================
-- Géographie des espaces : de quel département et de quelle région relève
-- chaque ville, pour que l'index /ville/ soit rangé par région.
--
-- 1. fr_departements : table de référence (source geo.api.gouv.fr, découpage
--    officiel 2026). Lecture publique, écriture réservée aux super admins :
--    c'est une référence administrative, pas un réglage de collectivité.
-- 2. city_branding.insee : code INSEE de la commune de l'espace. Rempli à la
--    création par demo-generate, rattrapé ici depuis demo_instances.
-- ============================================================================

create table if not exists public.fr_departements (
...
comment on table public.fr_departements is
  'Départements français et leur région (découpage officiel, source geo.api.gouv.fr).';

alter table public.fr_departements enable row level security;

drop policy if exists fr_departements_select on public.fr_departements;
create policy fr_departements_select on public.fr_departements
  for select to anon, authenticated using (true);

drop policy if exists fr_departements_write on public.fr_departements;
create policy fr_departements_write on public.fr_departements
  for all to authenticated using (public.is_global_admin()) with check (public.is_global_admin());

insert into public.fr_departements (code, nom, region_code, region_nom) values
...
-- Code INSEE de la commune de l'espace
alter table public.city_branding add column if not exists insee text;

comment on column public.city_branding.insee is
  'Code INSEE de la commune de l''espace : rattache la ville à son département et à sa région.';

-- Rattrapage : les espaces créés par la démo connaissent déjà leur commune
update public.city_branding b
   set insee = d.commune_insee
  from (
    select distinct on (lower(ville)) lower(ville) as ville, commune_insee
      from public.demo_instances
     where commune_insee is not null
     order by lower(ville), created_at desc
  ) d
 where lower(b.ville) = d.ville
   and b.insee is distinct from d.commune_insee;

create index if not exists city_branding_insee_idx on public.city_branding (insee);

-- La Métropole de Lyon n'est pas une commune : on la rattache à Lyon, ce qui
-- la range dans le Rhône et en Auvergne-Rhône-Alpes.
update public.city_branding set insee = '69123' where ville = 'metropole-lyon' and insee is null;
