...
Read `styles.css` and its `@import` closure (`tokens/colors.css`,
`tokens/typography.css`, `tokens/effects.css`, `tokens/gradients.css`,
`tokens/utilities.css`) for the exact values before styling.

## One idiomatic snippet

```jsx
// A page shell + a primary CTA card, on-brand.
