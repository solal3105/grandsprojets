...
test.describe('0.31 - Endpoints IA : préflight CORS', () => {

  for (const path of AI_ENDPOINTS) {
    test(`0.31.1 - RÉGRESSION : OPTIONS ${path} répond 204, pas 500`, async ({ request }) => {
    ...
    test(`0.31.2 - OPTIONS ${path} renvoie les en-têtes CORS`, async ({ request }) => {
      const res = await request.fetch(path, {
        method: 'OPTIONS',
        headers: { Origin: 'http://localhost:3001' },
      });
    ...
    test(`0.31.3 - ${path} : une origine hors allowlist ne se voit jamais renvoyée`, async ({ request }) => {
      const res = await request.fetch(path, {
        method: 'OPTIONS',
  ...
  test('0.31.4 - RÉGRESSION : OPTIONS /api/demo-lead répond 204, pas 500', async ({ request }) => {
...
test.describe('0.32 - Endpoints IA : méthode et authentification', () => {

  for (const path of AI_ENDPOINTS) {
    test(`0.32.1 - GET ${path} → 405`, async ({ request }) => {
      const res = await request.get(path);
      expect(res.status()).toBe(405);
      expect((await res.json()).error).toBe('Method not allowed');
    });

    test(`0.32.2 - POST ${path} sans en-tête Authorization → 401`, async ({ request }) => {
      const res = await request.post(path, { data: {} });
      expect(res.status()).toBe(401);
      expect((await res.json()).error).toBe('Unauthorized');
    });

    test(`0.32.3 - POST ${path} avec un jeton invalide → 401`, async ({ request }) => {
    ...
    test(`0.32.4 - POST ${path} avec un JWT bien formé mais non signé par Supabase → 401`, async ({ request }) => {
    ...
    test(`0.32.5 - ${path} : le corps d'erreur ne fuite jamais de détail interne`, async ({ request }) => {
      const res = await request.post(path, { data: {} });
