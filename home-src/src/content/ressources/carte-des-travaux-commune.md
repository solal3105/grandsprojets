...
Côté outils, tout dépend de vos moyens : un Google My Maps tenu à la main, le plugin carto de votre CMS, le SIG de l'interco si vous y avez accès. C'est aussi le problème que nous avons construit [Open Projets](https://openprojets.com/travaux) pour résoudre : une carte à vos couleurs, avec un module Travaux, des fiches mises à jour par un agent sans compétence technique, et des pages lisibles par les moteurs de recherche. Mais l'outil est secondaire : une carte gratuite tenue à jour vaudra toujours mieux qu'un bel outil abandonné.
