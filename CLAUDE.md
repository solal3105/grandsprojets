...
- Le `<title>` tient en **60 caractères**, un seul suffixe après la barre verticale : `| Open Projets` pour les pages du produit (home, cartes, demo, index des villes), `| {Nom de la collectivité}` pour les fiches. `fitTitle(nom, suffixe)` de `netlify/edge-functions/_lib/seo.js` sacrifie le suffixe, jamais le nom.
