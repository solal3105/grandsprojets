...
| **Site vitrine** (`/home-src/`, sources dans `src/v2/` + vues partagées `src/views/`, build prérendu dans `/home/`, servi à la racine `/`) | Vue 3 + Vite + Tailwind, build séparé. Pages : accueil, `/carte` `/travaux` `/participer` `/chantiers` `/diagnostic` (modules), `/tarification`, `/ressources`, `/a-propos`, `/aide`, `/confidentialite`, `/alternative-*`, `/helios` (noindex, hors menu) |
